...
async function signPayment(
  ...
  // AUTOMATON PROTECTION: Hard limit on auto-spending
  // 1.00 USDC = 1,000,000 atomic units
  const MAX_AUTO_SPEND = 1_000_000n;
  if (amount > MAX_AUTO_SPEND) {
    throw new Error(
      `Payment amount ${amount} exceeds safety limit of 1.00 USDC. Manual confirmation required.`,
    );
  }
