...
Your sandbox ID is ${identity.sandboxId}.`,
  ...
  // Layer 3.5: SCRATCHPAD (Mutable Memory)
  // This is CRITICAL for preventing amnesia across restarts.
  let scratchpad = "";
  try {
    const home = process.env.HOME || "/root";
    const spPath = path.join(home, ".automaton", "scratchpad.md");
    if (fs.existsSync(spPath)) {
      scratchpad = fs.readFileSync(spPath, "utf-8");
    }
  } catch { }

  if (scratchpad) {
    sections.push(
      `--- SCRATCHPAD (Your short-term memory & lessons) ---\n${scratchpad}\n--- END SCRATCHPAD ---`,
    );
  }
