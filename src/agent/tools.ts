  ...
  return [
    ...
    },
    {
      ...
      execute: async (args, ctx) => {
        ...
        const result = await ctx.conway.exec(
          `npm install -g ${pkg} --ignore-scripts`,
          60000,
        );
...
========================`;
    ...
    // ── Self-Mod: Install MCP Server ──
    {
      ...
      execute: async (args, ctx) => {
        const pkg = args.package as string;
        const result = await ctx.conway.exec(`npm install -g ${pkg} --ignore-scripts`, 60000);
    ...
    // ── Replication Tools ──
    {
      ...
      execute: async (args, ctx) => {
        ...
        // REPLICATION HARDENING: Inherit Memory (Lamarckian Evolution)
        // Read parent's scratchpad and append to child's genesis message
        try {
          ...
          if (existsSync(spPath)) {
            const memory = readFileSync(spPath, "utf-8");
            const heritage = `\n\n--- INHERITED MEMORY (FROM PARENT) ---\n${memory.slice(-2000)}\n--- END INHERITED MEMORY ---`;
            genesis.creatorMessage = (genesis.creatorMessage || "") + heritage;
          }
        } catch (err: any) {
          // Ignore memory inheritance errors, not fatal
        }
